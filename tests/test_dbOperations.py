"""
Тест-кейсы для модуля ./tests/dbOperations.py для pytest.
Запускать надо весь модуль целиком, поодиночке часть кейсов работать не будут, так как тестовая БД не будет создана/
наполнена.
Модуль использует fixtures из ./tests/conftest.py

Содержит функции:
    test_create_db_with_wrong_path(monkeypatch, temp_db_path)
    test_create_new_sqlite_db(monkeypatch, temp_db_path)
    test_create_new_table_in_sqlite_db(monkeypatch, temp_db_path)
    test_delete_user()
    test_delete_user_with_wrongId()
    test_get_nonexistent_user_preferences()
    test_get_user_preferences()
    test_insert_existing_user()
    test_insert_new_incorrect_user()
    test_insert_new_user()
    test_update_user_preferences()
    test_update_user_with_incorrect_preferences()
"""
...
def test_update_user_preferences():
    ...
    updatedUserPreferences = dbOperations.update_user_preferences(testConn, telegramId, city_update, excl_bar,
                                                                  excl_theme, excl_org)
...
def test_update_user_with_incorrect_preferences():
    ...
    updatedUserPreferences = dbOperations.update_user_preferences(testConn, telegramId, wrong_param, excl_bar,
                                                                  excl_theme, excl_org)
